...
> **Every essential Maven command — what it does, what it produces, and when to use it.**
...
Deletes the `target/` directory and all its contents — compiled classes, JARs, reports, and everything generated by previous builds.

```bash
mvn clean
```

**Output:** Empty project — no `target/` directory  
...
> ⚠️ If any test fails, the build stops here and no JAR is produced. This is intentional — broken tests block broken code from being shipped.

---

### `mvn package`

Runs `validate → compile → test → package` in sequence, producing a deployable artifact (JAR or WAR) in the `target/` directory.
...
**When to use:** Before sharing code with the team or merging to main — a full quality gate
...
**When to use:** To share your artifact with the team or publish to a company registry — typically run in CI/CD pipelines
...
mvn test
    │
    ├──▶ compile
    └──▶ Run tests in src/test/java/  ──▶  target/surefire-reports/

mvn package
    │
    ├──▶ compile
    ├──▶ test
    └──▶ Bundle into JAR/WAR  ──▶  target/your-app-1.0.jar
...
mvn deploy
    ...
    ├──▶ package
    ├──▶ install
    └──▶ Upload to remote repository (Nexus / Artifactory / GitHub Packages)
```
...
Compiles test code but does not execute tests. Use this sparingly — only when you need a quick artifact and tests are known to pass.
...
Tells Maven not to try downloading anything — uses only what's already in `~/.m2`. Useful in environments without internet access.
...
| `-B` | Batch mode — no interactive prompts (used in CI) |
| `-q` | Quiet mode — only show errors |
| `-X` | Debug mode — very verbose output for troubleshooting |
...
| **Complete local build** ⭐ | `mvn clean install` | JAR in `target/` + installed to `~/.m2` |
